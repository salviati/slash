...
size_t hash(const void *key, int len, unsigned int seed) {
	return (size_t)MurmurHash2(key, len, seed);
#ifdef __x86_64__
	return (size_t)MurmurHash64A(key, len, seed);
#else
	return (size_t)MurmurHash64B(key, len, seed);
#endif
}